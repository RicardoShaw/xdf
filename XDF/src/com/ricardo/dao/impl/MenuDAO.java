...
import com.ricardo.dao.IMenuDAO;
...
public class MenuDAO extends HibernateDaoSupport implements IMenuDAO{
	...
	public Menu findById(java.lang.Integer id) {
		...
		try {
			Menu instance = (Menu) currentSession().get(
					"com.ricardo.entity.Menu", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Menu> findByExample(Object instance) {
		log.debug("finding Menu instance by example");
		try {
			List<Menu> results = currentSession()
					.createCriteria("com.ricardo.entity.Menu")
	...
	public Menu merge(Menu detachedInstance) {
		...
		try {
			Menu result = (Menu) currentSession().merge(detachedInstance);
